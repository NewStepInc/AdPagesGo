...
let shareOptions = {
    title: "Ad Pages GO",
    message: "Ad Pages GO",
    url: "http://adpages.com",
    subject: "Ad Pages GO" //  for email
};

class Information extends BasicScreen {

    constructor(props) {
        super(props);

        this.state = {shareSheetVisible: false};

        this._onPrivacyPolicy = this._onPrivacyPolicy.bind(this);
        this._onLicenseAgreement = this._onLicenseAgreement.bind(this);
        this._onFacebook = this._onFacebook.bind(this);
        this._onTwitter = this._onTwitter.bind(this);
        this._onEmail = this._onEmail.bind(this);
        this._onShareApp = this._onShareApp.bind(this);
        this._onCloseShareSheet = this._onCloseShareSheet.bind(this);
    }
    ...
    _renderContentView() {
        return (
            <View style={styles.contentView}>
                ...
                {this._renderListPart()}
                {this._renderButtonPart()}

                <ShareSheet visible={this.state.shareSheetVisible} onCancel={this._onCloseShareSheet}>
                    <Button iconSrc={{ uri: TWITTER_ICON }}
                            onPress={()=>{
                              this._onCloseShareSheet();
                              setTimeout(() => {
    ...
    _renderListPart() {
        return (
            <View>
                <TouchableOpacity style={styles.listItem} onPress={() => this.props.navigator.push({id:Constants.SCREENS.ABOUT})}>
                    {this._renderListItem('About')}
                </TouchableOpacity>
                <TouchableOpacity style={styles.listItem} onPress={() => this.props.navigator.push({id:Constants.SCREENS.HELP})}>
                    {this._renderListItem('Help')}
                </TouchableOpacity>
                <TouchableOpacity style={styles.listItem} onPress={() => this.props.navigator.push({id:Constants.SCREENS.ACKNOWLEDGEMENT})}>
                    {this._renderListItem('Acknowledgement')}
                </TouchableOpacity>
                <TouchableOpacity style={styles.listItem} onPress={this._onPrivacyPolicy}>
                    {this._renderListItem('Privacy Policy')}
                </TouchableOpacity>
                <TouchableOpacity style={styles.listItem} onPress={this._onLicenseAgreement}>
                    {this._renderListItem('End-User License Agreement')}
                </TouchableOpacity>
                <TouchableOpacity style={styles.listItem} onPress={this._onEmail}>
                    {this._renderListItem('Email')}
                </TouchableOpacity>
                <TouchableOpacity style={styles.listItem} onPress={this._onShareApp}>
                    {this._renderListItem('Share the App!')}
                </TouchableOpacity>
            </View>
        );
    }

    _renderListItem(title) {
        let ret = [];
        ...
        ret.push((
            <Image key={title+'2'}
                style={{width: 12, height: 12, tintColor: 'lightgray', resizeMode:'contain', transform: [{rotateZ: '180deg'}]}}
    ...
    _renderButtonPart() {
        return (
            <View style={styles.buttonPart}>
                <TouchableOpacity onPress={this._onFacebook}>
                    <Image style={styles.socialButton}
                           source={require('image!btn_facebook')}/>
                </TouchableOpacity>
                <TouchableOpacity onPress={this._onTwitter}>
                    <Image style={styles.socialButton}
                           source={require('image!btn_twitter')}/>
                </TouchableOpacity>
            </View>
        )
    }
    ...
    _onPrivacyPolicy() {
        this.props.navigator.push({
            ...
            url: 'http://adpages.com/privacy-policy/'
        });
    }
    ...
    _onLicenseAgreement() {
        this.props.navigator.push({
            id: Constants.SCREENS.WEB,
            url: 'http://adpages.com/license-agreement/'
        });
    }
    ...
    _onFacebook() {
        this.props.navigator.push({
            id: Constants.SCREENS.WEB,
            url: 'https://www.facebook.com/Adpages'
        });
    }
    ...
    _onTwitter() {
        this.props.navigator.push({
            id: Constants.SCREENS.WEB,
            url: 'https://twitter.com/adpages'
        });
    }

    _onEmail() {
        let to = 'goapp@adpages.com';
        let subject = 'Ad Pages GO ' + (Constants.IS_ANDROID ? 'Android' : 'iOS') + ' App Feedback';
        Communications.email([to], [], [], subject, '');
    }
    ...
    _getScreenName() {
        return Constants.SCREENS.INFORMATION;
    }
}
...
const styles = StyleSheet.create({
    ...
    socialButton: {
        width: Constants.DIMENSION.WIDTH * 2 / 5,
        height: Constants.DIMENSION.WIDTH * 2 / 15,
        resizeMode: 'contain'
    },
    listItem: {
        left: 0,
